...
if (!isset($_SESSION['Email']) && !isset($_SESSION['rq'])) {
  ...
  $answer = $_SESSION['ra'];
  $query=mysqli_query($conn,"SELECT * FROM `user` WHERE `Recovery_Answer`='$answer'")or die(mysqli_error());
...
<body>
  <div class="container" style="position: relative; top: 0px;">
    <form action="insertnewpass_u.php" method="POST" class="login-email">
